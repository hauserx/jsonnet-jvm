...
public class JavaTranspiler extends JsonnetBaseVisitor<String> {
    ...
    private void generateMethod(JsonnetParser.BindContext bind) {
        ...
        if (bind.params() != null) {
            for (JsonnetParser.ParamContext param : bind.params().param()) {
                ...
                bodyPreamble.append("        final Val ").append(paramName).append(" = (").append(rawParamName).append(" != null) ? ").append(rawParamName).append(" : ");
        ...
        methods.append("    public static Val ").append(methodName).append("(")
    ...
    public String visitObject(JsonnetParser.ObjectContext ctx) {
        ...
        for (JsonnetParser.MemberContext member : inside.member()) {
            if (member.field() != null) {
                ...
                code.append("            ").append(varName).append(".addField(\"").append(key).append("\", () -> ").append(valueExpr).append(");\n");
            }
        }
    ...
    public String visitStringLit(JsonnetParser.StringLitContext ctx) {
        ...
        if (text.startsWith("'")) {
            ...
            // We need to escape " inside the content if it was unescaped (valid in single quoted jsonnet)
