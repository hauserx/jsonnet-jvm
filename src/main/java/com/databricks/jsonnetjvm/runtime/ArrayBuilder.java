...
public class ArrayBuilder {
  ...
  private void addDouble(double d) {
    ...
    if (doubles == null) doubles = new double[capacity];
    if (size >= doubles.length) doubles = Arrays.copyOf(doubles, doubles.length * 2);
    doubles[size] = d;
  }
  ...
  private void addBoolean(boolean b) {
    ...
    if (booleans == null) booleans = new boolean[capacity];
    if (size >= booleans.length) booleans = Arrays.copyOf(booleans, booleans.length * 2);
    booleans[size] = b;
  }
  ...
  private void addString(String s) {
    ...
    if (strings == null) strings = new String[capacity];
    if (size >= strings.length) strings = Arrays.copyOf(strings, strings.length * 2);
    strings[size] = s;
  }
  ...
  private void ensureGeneric() {
    ...
    if (doubles != null) {
      for (int i = 0; i < size; i++) generic.add(doubles[i]);
      doubles = null;
    } else if (booleans != null) {
      for (int i = 0; i < size; i++) generic.add(booleans[i]);
      booleans = null;
    } else if (strings != null) {
      for (int i = 0; i < size; i++) generic.add(strings[i]);
      strings = null;
    }
  }

  public JArray build() {
    if (generic != null) return new JGenericArray(generic);
    if (doubles != null) {
